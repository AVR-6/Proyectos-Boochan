...
2. Para copiar tu llave pública al Ubuntu de forma automática, usa este comando desde tu PC:
   ssh-copy-id iker@ssh.boochan.space
...
### 🔑 S6.2.1: Interoperabilidad y Gestión de Clientes (Bitvise/PuTTY)
...
### 🔑 S6.2.2: Creación de la key con la herramienta Bitvise SSH
1. **Client key manager:** Accedemos a este apartado dentro de login
 ...
 3. **Exportacion de Bitvise:** Aplicaremos el formato OpenSSH y exportaremos a nuestra carpeta .ssh
     ...
     - Una vez tengamos la key publica en formato .pub, la editaremos como en pasos anteriores y la pegaremos en la carpeta authorized_keys dentro del servidor.
			```nano ~/.ssh/authorized_keys```
		Seguidamente editaremos "/etc/ssh/sshd_config" y dejarlos como en las siguientes lineas
		```
		PasswordAuthentication no
		PubkeyAuthentication yes
		```
...
3. En **Zone Resources**, selecciona tu dominio: `boochan.space`.
    
4. Copia el Token generado (es el código largo que usaremos ahora).
...
```
sudo certbot certonly --dns-cloudflare --dns-cloudflare-credentials /root/.cloudflare_api_token -d boochan.space -d ssh.boochan.space
```
