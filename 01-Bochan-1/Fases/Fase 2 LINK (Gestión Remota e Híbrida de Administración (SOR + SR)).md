...
1. **Instalar OpenSSH Server:**
	-  Para actualizar la información del sistema sobre los paquetes disponibles `sudo apt update`
		...
		- Tenemos que entrar en la configuración del ssh para ello entraremos en el fichero con el comando `sudo nano /etc/ssh/sshd_config`. Una vez dentro buscamos una línea en la que pone `Port 22` y la descontentamos quitándole la #. Nos dirigimos al final del fichero y escribimos los siguientes parámetros:
			```
			- ListenAddress  (Tu_ip) #en mi caso en la fase 1 le he asignado a la maquina la 192.168.1.200
			- PermitRootLogin no
			- MaxAuthTries 3 #Numero de intentos para el ssh
			- LoginGraceTime 60 #Los segundos que tiene el usuario para poner la contraseña
		...
		- Para comprobar la sintaxis ponemos el comando `sudo sshd -t` (Si funciona bien no devuelve nada). ==Si aparece el mensaje de Missing privilege separation directory: /run/sshd = el demonio necesita un directorio para resolver esto pondremos:==
		...
		- Si todo funciona bien es la hora de reiniciar `sudo systemctl restart ssh`
...
3. **** Instalar el software AnyDesk, configurar contraseña de acceso no presencial en AnyDesk en el sistema anfitrión :
